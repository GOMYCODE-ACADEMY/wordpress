<?php
/**
 * Podcaster Radio functions and definitions
 *
 * @package Podcaster Radio
 */

/* Theme Setup */
if ( ! function_exists( 'podcaster_radio_setup' ) ) :
 
function podcaster_radio_setup() {

	$GLOBALS['content_width'] = apply_filters( 'podcaster_radio_content_width', 640 );
	
	load_theme_textdomain( 'podcaster-radio', get_template_directory() . '/languages' );
	...
	add_image_size('podcaster-radio-homepage-thumb',240,145,true);
	
    register_nav_menus( array(
		'primary' => __( 'Primary Menu', 'podcaster-radio' ),
	) );
	 ...
	 */
	add_editor_style( array( 'css/editor-style.css', podcaster_radio_font_url() ) );

	// Theme Activation Notice
	global $pagenow;

	if (is_admin() && ('themes.php' == $pagenow) && isset( $_GET['activated'] )) {
		add_action('admin_notices', 'podcaster_radio_activation_notice');
	}
}
endif;

add_action( 'after_setup_theme', 'podcaster_radio_setup' );

// Notice after Theme Activation
function podcaster_radio_activation_notice() {
	echo '<div class="notice notice-success is-dismissible welcome-notice">';
	echo '<p>'. esc_html__( 'Thank you for choosing Podcaster Radio. Would like to have you on our Welcome page so that you can reap all the benefits of our Podcaster Radio.', 'podcaster-radio' ) .'</p>';
	echo '<p><a href="'. esc_url( admin_url( 'themes.php?page=podcaster_radio_guide' ) ) .'" class="button button-primary">'. esc_html__( 'GET STARTED', 'podcaster-radio' ) .'</a></p>';
	echo '</div>';
}

/* Theme Widgets Setup */
function podcaster_radio_widgets_init() {
	register_sidebar( array(
		'name'          => __( 'Blog Sidebar', 'podcaster-radio' ),
		'description'   => __( 'Appears on blog page sidebar', 'podcaster-radio' ),
		'id'            => 'sidebar-1',
	...
	register_sidebar( array(
		'name'          => __( 'Page Sidebar', 'podcaster-radio' ),
		'description'   => __( 'Appears on page sidebar', 'podcaster-radio' ),
		'id'            => 'sidebar-2',
		'before_widget' => '<aside id="%1$s" class="widget mb-5 p-3 %2$s">',
		'after_widget'  => '</aside>',
		'before_title'  => '<h3 class="widget-title py-3 px-3">',
	...
	register_sidebar( array(
		'name'          => __( 'Footer Navigation 1', 'podcaster-radio' ),
		'description'   => __( 'Appears on footer 1', 'podcaster-radio' ),
		'id'            => 'footer-1',
	...
	register_sidebar( array(
		'name'          => __( 'Footer Navigation 2', 'podcaster-radio' ),
		'description'   => __( 'Appears on footer 2', 'podcaster-radio' ),
		'id'            => 'footer-2',
		'before_widget' => '<aside id="%1$s" class="widget py-3 %2$s">',
		'after_widget'  => '</aside>',
		'before_title'  => '<h3 class="widget-title px-0 py-2">',
	...
	register_sidebar( array(
		'name'          => __( 'Footer Navigation 3', 'podcaster-radio' ),
		'description'   => __( 'Appears on footer 3', 'podcaster-radio' ),
		'id'            => 'footer-3',
		'before_widget' => '<aside id="%1$s" class="widget py-3 %2$s">',
		'after_widget'  => '</aside>',
		'before_title'  => '<h3 class="widget-title px-0 py-2">',
	...
	register_sidebar( array(
		'name'          => __( 'Footer Navigation 4', 'podcaster-radio' ),
		'description'   => __( 'Appears on footer 4', 'podcaster-radio' ),
		'id'            => 'footer-4',
		'before_widget' => '<aside id="%1$s" class="widget py-3 %2$s">',
		'after_widget'  => '</aside>',
		'before_title'  => '<h3 class="widget-title px-0 py-2">',
	...
	register_sidebar( array(
		'name'          => __( 'Shop Page Sidebar', 'podcaster-radio' ),
		'description'   => __( 'Appears on shop page', 'podcaster-radio' ),
	...
	register_sidebar( array(
		'name'          => __( 'Single Product Sidebar', 'podcaster-radio' ),
		'description'   => __( 'Appears on single product page', 'podcaster-radio' ),
...
add_action( 'widgets_init', 'podcaster_radio_widgets_init' );

/* Theme Font URL */
function podcaster_radio_font_url() {
	$font_url      = '';
	$font_family   = array();
...
function podcaster_radio_scripts() {
	wp_enqueue_style( 'podcaster-radio-font', podcaster_radio_font_url(), array() );
	wp_enqueue_style( 'podcaster-radio-block-style', get_theme_file_uri('/assets/css/blocks.css') );
	...
	wp_enqueue_style( 'podcaster-radio-basic-style', get_stylesheet_uri() );
	wp_style_add_data('podcaster-radio-basic-style', 'rtl', 'replace');
	/* Inline style sheet */
	require get_parent_theme_file_path( '/custom-style.php' );
	wp_add_inline_style( 'podcaster-radio-basic-style',$podcaster_radio_custom_css );
	...
	wp_enqueue_script( 'podcaster-radio-custom-scripts-jquery', esc_url(get_template_directory_uri()) . '/assets/js/custom.js', array('jquery'),'' ,true );
	wp_enqueue_script( 'podcaster-radio-wow-jquery', esc_url(get_template_directory_uri()) . '/assets/js/wow.js', array('jquery'),'' ,true );
...
add_action( 'wp_enqueue_scripts', 'podcaster_radio_scripts' );

define('PODCASTER_RADIO_FREE_THEME_DOC',__('https://www.vwthemesdemo.com/docs/free-podcaster-radio/','podcaster-radio'));
define('PODCASTER_RADIO_SUPPORT',__('https://wordpress.org/support/theme/podcaster-radio/','podcaster-radio'));
define('PODCASTER_RADIO_REVIEW',__('https://wordpress.org/support/theme/podcaster-radio/reviews/','podcaster-radio'));

function podcaster_radio_sanitize_dropdown_pages( $page_id, $setting ) {
...
function podcaster_radio_sanitize_choices( $input, $setting ) {
    global $wp_customize; 
...
function podcaster_radio_sanitize_number_range( $number, $setting ) {
...
function podcaster_radio_sanitize_number_absint( $number, $setting ) {
...
/* Excerpt Limit Begin */
function podcaster_radio_string_limit_words($string, $word_limit) {
...
if ( ! function_exists( 'podcaster_radio_switch_sanitization' ) ) {
	function podcaster_radio_switch_sanitization( $input ) {
		if ( true === $input ) {
			return 1;
		} else {
			return 0;
...
add_filter('loop_shop_columns', 'podcaster_radio_loop_columns');
	if (!function_exists('podcaster_radio_loop_columns')) {
		function podcaster_radio_loop_columns() {
		return 3; // 3 products per row
	}
}

function podcaster_radio_logo_title_hide_show(){
	if(get_theme_mod('podcaster_radio_logo_title_hide_show') == '1' ) {
		return true;
	}
	return false;
}

function podcaster_radio_tagline_hide_show(){
	if(get_theme_mod('podcaster_radio_tagline_hide_show') == '1' ) {
		return true;
	}
	return false;
}
