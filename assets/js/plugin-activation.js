jQuery(document).ready(function($) {
    'use strict';
    var this_obj = podcaster_radio_plugin_activate_plugin;

    $('#gutenberg_editor .plugin-activation-redirect a').addClass('ive-redirect-to-dashboard');

    $(document).on('click', '.podcaster-radio-plugin-install', function(event) {
        ...
        wp.updates.installPlugin({
            ...
            success: function(data) {
                ...
                button.removeClass('button-secondary updating-message podcaster-radio-plugin-install');
                button.addClass('button-primary podcaster-radio-plugin-activate');
                button.trigger('click');
            },
            error: function(data) {
                jQuery('.podcaster-radio-recommended-plugins .podcaster-radio-activation-note').css('display','block');
    ...
    $(document).on('click', '.podcaster-radio-plugin-activate', function(event) {
    ...
    jQuery('#lite_theme .ibtana-skip-btn').click(function(){
        ...
        jQuery('#lite_theme .podcaster-radio-recommended-plugins').css('display','none');
    });

    jQuery('#block_pattern .ibtana-skip-btn').click(function(){
        var light_theme = jQuery(this).attr('get-start-tab-id');
        jQuery('.' + light_theme).css('display','block');
        jQuery('#block_pattern .podcaster-radio-recommended-plugins').css('display','none');
    });
